...
function verDetalles(id) {
    ...
    sliderContainer.style.overflow = 'hidden'; // Esto corta los excesos de la cinta larga
    // --- 1. LIMPIEZA TOTAL DE BADGES ---
    // Borramos cualquier rastro para que no se dupliquen si los IDs estaban fallando
    const viejosBadges = sliderContainer.querySelectorAll('.badge-popular, .badge-new-pro, .badge-status, .badge-agotado-premium');
    viejosBadges.forEach(b => b.remove());

    // --- 2. LÓGICA DE BADGES EN EL MODAL ---
    let badgesModalHTML = '';
    const estaAgotado = p.stock === false;
    ...
    // --- 3. CARGAR NOMBRE, MARCA Y MODELO ---
    ...
    // --- 4. LÓGICA DE PRECIOS (Cambia a gris si está agotado) ---
const precioCont = document.getElementById('detalles-precio');
// Definimos el color: gris si está agotado, azul si hay stock
const colorPrecio = estaAgotado ? '#888' : 'var(--azul-soniline)';
...
if (p.precioAnterior) {
    precioCont.innerHTML = `
        ...
        <span style="color: ${colorPrecio}; font-weight: bold;">$${p.precio}</span>
    `;
} else {
    precioCont.innerHTML = `<span style="color: ${colorPrecio}; font-weight: bold;">$${p.precio}</span>`;
}
    
   // --- 4. Cargar Imágenes al Slider con efecto gris si está agotado ---
wrapper.innerHTML = '';
const fotos = p.imagenes && p.imagenes.length > 0 ? p.imagenes : [p.imagen || 'img/default.jpg'];

// Determinamos si aplicamos la clase de escala de grises
const claseGris = estaAgotado ? 'img-detalles-agotado' : '';

fotos.forEach(img => {
    wrapper.innerHTML += `
        <img src="${img}" 
             class="${claseGris}" 
             style="width: 100%; flex-shrink: 0; object-fit: contain; aspect-ratio: 1/1;" 
...
wrapper.style.transform = `translateX(0)`;
    ...
    // --- 7. ESPECIFICACIONES (CORREGIDAS PARA TEMA CLARO) ---
    ...
    if (p.specs) {
        ...
        for (let key in p.specs) {
            htmlSpecs += `
                <li style="border-bottom: 1px solid var(--borde); padding: 10px 0; display: flex; justify-content: space-between; font-size: 0.9rem;">
                    <span style="color: var(--texto-secundario);">${key}:</span>
                    <span style="font-weight: bold; color: var(--texto-principal);">${p.specs[key]}</span>
                </li>`;
        }
        htmlSpecs += '</ul>';
    } else {
        htmlSpecs += '<p style="color: var(--texto-secundario); font-style: italic;">No hay especificaciones adicionales.</p>';
    }
